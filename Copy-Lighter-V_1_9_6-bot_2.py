...
class CopyTradingBot:
    ...
    def _apply_our_fill_to_positions(self, fill):
        ...
        try:
            ...
            # If fill has a client_order_index from a different bot, IGNORE IT
            # This prevents bot B from closing bot A's positions
            if client_order_index > 0:  # client_order_index is set
                if client_order_index < self.BOT_TAG or client_order_index >= self.BOT_TAG + 1_000_000:
                    # This fill is from a different bot (different tag range)
                    # DO NOT process it - return silently
                    return
