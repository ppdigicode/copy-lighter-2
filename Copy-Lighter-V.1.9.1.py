...
class CopyTradingBot:
    ...
    def __init__(self):
        print("\n🤖 Lighter Trading Bot v1.9.1 - Fixed Orphan Reconciliation + safety flatten + Timeout API")
    ...
    def _check_orphan_positions(self):
        """
        SIMPLE VERSION - Use normal order placement!
        
        If target is flat but we have a position, close it using
    ...
    def _place_order_internal(self, market_index, is_buy, size, price, is_closing=False, target_fill_time_ms=None):
        ...
        with self.state_lock:
            ...
            # Store target fill timestamp and intended price for latency/slippage calculation
            if target_fill_time_ms:
                self.order_target_timestamps[market_index] = target_fill_time_ms
                self.order_intended_prices[market_index] = price  # Store price AFTER slippage adjustment
    ...
    def _apply_our_fill_to_positions(self, fill):
        ...
        try:
            ...
            self._add_fill_to_buffer(market_index, delta, price, side, fill)

        except Exception as e:
            print(f"⚠️  Error applying our fill: {e}")
    
    def _add_fill_to_buffer(self, market_index, delta, price, side, fill):
        """Add fill to buffer and start/reset timer for coalescing"""
        with self.state_lock:
    ...
    def _flush_pending_fills(self, market_index):
        ...
        with self.state_lock:
            ...
            if is_opening:
                # Opening or adding to position - update entry price
        ...
        # Calculate slippage if we have intended price
        slippage_str = ""
        if intended_price and intended_price > 0:
            slippage_pct = ((avg_price - intended_price) / intended_price) * 100
            if side == 'SELL':
                # For SELL, negative slippage is good (sold higher than intended)
        ...
        if position_closed and is_closing:
            ...
            if entry_cost > 0:
                ...
                target_exit_cost = self.target_exit_costs.get(market_index, 0)
                
                if target_entry_cost > 0 and target_exit_cost > 0:
                    # Calculate target's actual PnL using their real exit prices
                    if current > 0:  # We were LONG (so target was too)
                        target_pnl = target_exit_cost - target_entry_cost
                    else:  # SHORT
                        target_pnl = target_entry_cost - target_exit_cost
                    
                    # Calculate our % of target's PnL
                    pnl_ratio = (our_pnl / target_pnl * 100) if target_pnl != 0 else 0
                    
                    pnl_sign = "+" if our_pnl >= 0 else ""
                    target_pnl_sign = "+" if target_pnl >= 0 else ""
